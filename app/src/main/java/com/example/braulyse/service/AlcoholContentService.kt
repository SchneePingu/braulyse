...
private const val BALLING_FACTOR = 0.81f // [1]
...
class AlcoholContentService {
    companion object {
        fun calculateAlcoholContent(measurement: DensityMeasurement): Float {
            val initialMassFraction = DensityMeasurementService.toMassFraction(measurement.initialDensity)
            val finalMassFraction = DensityMeasurementService.toMassFraction(measurement.finalDensity)
            return (initialMassFraction - finalMassFraction) * BALLING_FACTOR /
                    ((EXTRACT_AMOUNT - YEAST_CARBONIC_ACID_MASS_FRACTION * initialMassFraction) * ETHANOL_DENSITY)
        }
    }
}